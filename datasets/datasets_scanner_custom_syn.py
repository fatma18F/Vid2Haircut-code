...
class HairstyleRealDatasetScanner(HairstyleRealDataset):
    ...
    def use_depth(self, idx, flip=False):
        ...
        depth_filename = self.hair_list[idx].replace('.jpeg', '.jpeg.npz')#.replace('.jpg', '.npz')
    ...
    def load_gaus(self, sample_path, flip=False):
        ...
        orient_path = os.path.join(self.root_path, 'orientation_maps', sample_path.replace('.jpeg','.png'))
        ...
        depth_path = os.path.join(self.root_path, 'depth_apple_pro', sample_path.replace('.jpeg', '.npz'))
        #/home/ayed/prerocessing_input_monocular_hair_modelling/sim_data/data/Wings/depth_apple_pro512_aligned_aligned/NewLevelSequence.camera_turnaround.0000.jpeg.npz
        ...
        #cam_path = os.path.join(self.root_path, 'proj_matx_inv', sample_path.replace('.png', '.txt'))
        #cam= torch.tensor(cam_path )         # shape (3, 4) or (4, 4)
        cam =np.load('/home/ayed/prerocessing_input_monocular_hair_modelling/sim_data/simulations/extrs.npy')[index] #0
...
#         combine gabor map with direction map from hairstep
        ...
        if not self.use_orientations2:
             return feats_gaus, cam_gaus, 0
        
        else:
                
            orient_path = os.path.join(self.root_path, 'best_ori2' , sample_path)
            o = cv2.imread(orient_path, cv2.IMREAD_GRAYSCALE)
            o = (180-o)/180 
            #orient_angle2 = PILtoTorch(Image.open(orient_path), resolution=None, max_value=max_value)
            o = torch.from_numpy(np.array(o))  
            gt_orient_angle2=o.unsqueeze(dim=-1).permute(2, 0, 1)
   
            #confidence = PILtoTorch(Image.open(conf_orient_path), resolution=None,  max_value=255)
            c = cv2.imread(conf_orient_path, cv2.IMREAD_GRAYSCALE) / 255.
            confidence = torch.from_numpy(np.array(c))  
            confidence=confidence.unsqueeze(dim=-1).permute(2, 0, 1)

            gt_orient2 = torch.cat([
                gt_orient_angle2, confidence
            ], dim=0)

            return feats_gaus, cam_gaus, gt_orient2
